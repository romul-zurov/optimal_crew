...
const ORDER_PRIGLASITE_KLIENTA = 31; // согласно ORDER_STATES

const ORDER_KLIENT_NE_VYSHEL = 32; // согласно ORDER_STATES

const ORDER_ZAKAZ_POLUCHEN = 33; // согласно ORDER_STATES

const ORDER_VODITEL_OTKAZALSYA = 34; // согласно ORDER_STATES
...
begin
	...
	begin
		...
		for pp in self.speed_list do
			try
				sp := TSpeed(pp).speed();
				if (sp > 0.5) and (sp < 200.0) then // отбрасываем глючные и нулевые скорости
				begin
					result := result + sp;
					inc(cou);
				end;
